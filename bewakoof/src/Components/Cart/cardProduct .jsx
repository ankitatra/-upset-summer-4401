	...
	return (
		<Box border="1px solid rgb(234, 234, 234)" borderRadius={"5px"} mt={5}>
			...
			>
				<Box width={["60%", "80%"]} mx={3} mt={["0.1rem", "-20px"]}>
					...
					<HStack mt={["1rem", "0.1rem"]}>
						<Text fontSize={"18px"} fontWeight="bold">
							₹ {data[0].discountedPriceText}
						</Text>
						<Text fontSize={"14px"} color="gray" as="del">
							₹{data[0].actualPriceText}
						</Text>
					</HStack>
					<Text color={"#1d8802"} textAlign={"left"} fontSize={["13px", "15px"]}>
						{`You saved ₹${data[0].discount_price_box}!`}
					</Text>
				...
				{/* image box */}
				<Box width={["40%", "20%"]}>
					<Image mt={["-1rem", "0rem"]} borderRadius="6px" src=
						{data[0].productImgTagSrc}/>
				</Box>
			</HStack>
